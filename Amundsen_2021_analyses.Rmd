...
```{r map-CTD-locations, echo=F, fig.align='center', fig.height=4, fig.width=9}
# Map
map_summary <- CTD_1m %>%
  group_by(CTD_id) %>%
  ...
  scale_x_continuous("Longitude (°E)", breaks=c(-85.1,-80,-75,-69.9), 
                     labels=c(-85,-80,-75,-70), expand=c(0,0)) +
  # Latitude lines
  ...
  scale_y_continuous("Latitude (°N)", breaks=c(74,75.025,76.055,77.09,78.12), 
                     labels=seq(74,78,1), expand=c(0,0)) +
  # Plot 500, 1000, and 2000m isobath
  geom_contour(data=bathy, aes(x=lon, y=lat, z=depth), breaks=-500,
               size=0.3, colour="grey70", lty=1) +
  # geom_tile(data=bathy, aes(x=lon, y=lat, fill=depth_d)) +
  # Plot coastline and glaciers
  geom_polygon(data=coast, aes(x=lon, y=lat, group=group), fill="grey20", col="transparent") +
  # geom_polygon(data=glacier, aes(x=lon, y=lat, group=group), fill="grey90") +
  geom_polygon(data=coast, aes(x=lon, y=lat, group=group), fill="transparent", col="black", size=0.1) +
  # Plot CTD casts locations
  geom_point(aes(x=lon, y=lat), col="red", size=2) +
  scale_color_discrete("CTD cast") +
  scale_fill_cmocean(name="deep", discrete=T) +
  # Change map projection
  coord_map("azequidistant", ylim=c(74.5,78.8), xlim=c(-89,-68)) +
  # coord_cartesian(ylim=c(74,79), xlim=c(-86,-67)) +
  theme(axis.text=element_text(), axis.ticks=element_line(), legend.title=element_text(hjust=0.5),
        panel.border=element_rect(fill=NA))

# Photo CTD rosette
img_path <- readPNG("photos/DSCF5960_rosette_downward.png")

# Plot
plot_grid(map_summary, ggdraw() + draw_image(img_path, scale=0.9), labels="AUTO", label_size=9)
```

**Figure  1. A** Locations of the CTD casts in northern Baffin Bay (colored dots). The solid grey line represents the 500 m isobath. **`r text_col("Make sure that the map has the WBAT stations instead of the CTD and polygons to define the areas. Summary table of each DE station?","red")`** **B** WBAT with sideward facing transducers, UVP5. and UVP6 installed on the CTD rosette. 
...
```{r supplementary-table-echosounder, echo=F}
# Create table
