...
To check the signal to noise ratio (over the range) we used the WBAT profile of station BB2 because there was some sections of the water column with very few scatterers.

```{r plot-SD-Sv, fig.width=7, fig.height=9}
WBAT %>%
  # Calculate standard deviation of Sv corrected over each 1m range bin
  group_by(frequency, cast_number, range) %>%
  summarise(mean_Sv = 10 * log10(mean(10 ^ (Sv_cor / 10))),
            sd_Sv = 10 * log10(sd(10 ^ (Sv_cor / 10)))) %>%
  ungroup() %>%
  mutate(cast_number = paste0("CTD ", as.character(cast_number))) %>%
  ggplot() +
  geom_path(aes(x = range, y = sd_Sv, col = frequency)) +
  facet_rep_wrap(~cast_number, ncol = 2) +
  scale_x_continuous("Range (m)", limits = c(0, 100), breaks = seq(0, 100, 10)) +
  scale_y_continuous(expression("Standard deviation S"[V]*" (dB re 1 m"^2*")")) +
  scale_color_discrete("Frequency (kHz)") + 
  theme(legend.position = c(0.75, 0.1))
```

SD slightly increasing with range at 333 (to be expected?) because of SNR?
...
p_CTD <- CTD %>%
  ...
  scale_x_continuous("temp", breaks = seq(-2, 10, 0.5),
                     sec.axis = sec_axis(~ (. - a)/b, name = "sal", breaks = seq(0, 40, 1))) +
...
# WBAT
p_WBAT <- WBAT %>%
  filter(cast_number == 2 & between(range, 5, 95)) %>%
  # Calculate median Sv over the 5 - 95 m range
  group_by(depth, frequency) %>%
  summarise(Sv_median = 10 * log10(median(10 ^ (Sv_cor / 10)))) %>%
  ggplot() +
  geom_path(aes(x = Sv_median, y = depth, col = frequency)) +
...
# UVP
p_UVP <- UVP %>%
  filter(station == "DE110" & cast_number == 2) %>%
  ggplot() +
