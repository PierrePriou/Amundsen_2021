...
CTD %>%
  ...
  scale_x_continuous("temp", breaks = seq(-2, 10, 0.5),
                     sec.axis = sec_axis(~ (. - a)/b, name = "sal", breaks = seq(0, 40, 0.5))) +
  # geom_path(aes(x = fluo, y = depth, group = cast_number), col = "green") +
...
WBAT %>%
  ...
  summarise(Sv_median = median(10 ^ (Sv_mean / 10)),
            NASC = sum(NASC)) %>%
  mutate(Sv_median = 10 * log10(Sv_median)) %>%
  ggplot() +
...
UVP %>%
  ...
  ggplot() +
  geom_jitter(aes(x = taxa, y = depth_UVP, col = taxa), height = 0, width = 0.1, alpha = 0.5, na.rm = T) +
  scale_y_reverse(limits = c(265, 0), breaks = seq(0, 1500, 50))
```
