...
WBAT <- WBAT_bin %>%
  ...
  mutate(keep = case_when(frequency == 38 ~ if_else(between(range, 10, 95), T, F),
                          frequency == 333 ~ if_else(between(range, 10, 60), T, F)))
```
...
```{r COPS-loading}
COPS_raw <- read_csv("data/C-OPS/C-OPS_fitted_processed.csv", show_col_types = F,
                     locale = locale(tz = "UTC"))
```
...
```{r COPS-tidying}
COPS <- COPS_raw %>%
  # Select stations and wavelengths of interest
  filter(station %in% c("DE110", "DE320", "DE420") &
           between(lambda_nm, 400, 690)) %>%
  # Fix variable type
  mutate(station = factor(station),
  ...
  group_by(station, COPS_ID, ID, depth, lambda_nm) %>%
  summarise(kz = mean(kz, na.rm = T),
            edz = mean(ed_uW_cm2, na.rm = T)) %>%
  ungroup()
```

To extrapolate irradiance at the depth of the DSL I need to use the appropriate coefficient of extinction (kz) (Figure 3). 
...
col_pal <- w_length2rgb(c(412, 443, 465, 490, 510, 532, 555, 560, 589,
                          625, 665, 683)) %>% 
  as_tibble() %>%
  as.matrix()
...
*Figure 3.* Vertical profile of (a) downwelling irradiance (Ed in µW cm^-2^ nm^-1^) and (b) coefficient of extinction (kz in m^-1^) between 400 and 700 nm (colored lines), and (c) fluorescence (arbitrary units) at each station. The dashed red rectangles in (c) indicate the depth ranges where light data are available. Note the different y-axis scale for (c) the fluorescence. 

For each wavelength, we calculated the mean kz (k~mean~) of the last 5 m of the profile. To extrapolate irradiance at mesopelagic depth, we used both k~mean~ and the deepest kz (k~max~) available at each wavelength. 
```{r kz-calulation}
# Mean kz over the last 5 m
kz_mean <- COPS %>% 
  # Remove NAs
  filter(is.na(kz) == F) %>%
  # Find last 5 m of each wavelength at each station
  group_by(station, COPS_ID, ID, wavelength) %>%
  mutate(keep = if_else(max(depth) - depth <= 5, T, F)) %>%
  filter(keep == T) %>%
  summarise(kz_mean = mean(kz)) %>%
  ungroup() %>%
  # Keep relevant variables
  dplyr::select(station, COPS_ID, ID, wavelength, kz_mean)
...
kz_max_depth <- COPS %>% 
  ...
  # Find last 5 m of each wavelength at each station
  group_by(station, COPS_ID, ID, wavelength) %>%
  mutate(keep = if_else(max(depth) == depth, T, F)) %>%
  filter(keep == T) %>%
  rename(kz_max_depth = kz) %>%
  ungroup() %>%
  # Keep relevant variables
  dplyr::select(station, COPS_ID, ID, wavelength, kz_max_depth)
```

We extrapolated the ambient irradiance Ed~z~ down to the seafloor using the following equation:
$$Ed_{z} = Ed_{0} . e ^ {-k . z}$$

where Ed~z~ is the downwelling irradiance at depth z (in µW cm^-2^ nm^-1^), Ed~0~ the downwelling irradiance just below the surface in (µW cm^-2^ nm^-1^), k is either the k~mean~ or k~max~ (in m^-1^), and z is the depth (in m).

```{r light-extratpolation}
COPS_extra <- COPS %>%
  # Complete dataset to 602 m
  group_by(station, COPS_ID, ID, wavelength, mean_edz_0m) %>%
  complete(depth = seq(0, 602, 1)) %>%
  ungroup() %>%
  # Remove data below seafloor
  mutate(remove = case_when(station == "DE110" & depth > 260 ~ T,
                            station == "DE320" & depth > 575 ~ T)) %>%
  filter(is.na(remove) == T) %>%
  dplyr::select(-remove) %>%
  # Add kz mean and kz at max depth
  left_join(., kz_mean, by = c("station", "COPS_ID", "ID", "wavelength")) %>%
  left_join(., kz_max_depth, by = c("station", "COPS_ID", "ID", "wavelength")) %>%
  # Extrapolate edz
  mutate(edz_kz_mean = mean_edz_0m * exp(- kz_mean * depth),
         edz_kz_max = mean_edz_0m * exp(- kz_max_depth * depth))

# Remove unused data
rm(kz_max_depth, kz_mean)
```
